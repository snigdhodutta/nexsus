...
subscribers map[string]map[string]pool.Connection
mu          sync.RWMutex
closed      chan struct{}
wg          sync.WaitGroup
}
...
subscribers: make(map[string]map[string]pool.Connection),
...
func (s *Server) Subscribe(conn pool.Connection, topics ...string) error {
s.mu.Lock()
defer s.mu.Unlock()

for _, topic := range topics {
if s.subscribers[topic] == nil {
s.subscribers[topic] = make(map[string]pool.Connection)
}
...
func (s *Server) Unsubscribe(conn pool.Connection, topics ...string) error {
s.mu.Lock()
defer s.mu.Unlock()
...
if err := conn.Send(convertToPoolMessage(msg)); err != nil {
continue
}
}
}
...
func (s *Server) GetConnection(id string) (pool.Connection, error) {
...
func (s *Server) AddConnection(conn pool.Connection) error {
...
for _, subs := range s.subscribers {
for _, conn := range subs {
_ = conn.Send(convertToPoolMessage(pingMsg))
}
}
}
...
func convertToPoolMessage(msg *Message) *pool.Message {
return &pool.Message{
ID:        msg.ID,
Topic:     msg.Topic,
Payload:   msg.Payload,
